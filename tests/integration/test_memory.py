...
async def test_upsert_then_semantic_recall_and_forget(pg_url):
    ...
    async def fake_embed(texts):
        table = {"learning guitar": [1.0, 0.0], "hates cold weather": [0.0, 1.0],
                 "guitar": [1.0, 0.0]}
        return [_pad(table.get(t, [0.5, 0.5])) for t in texts]
...
async def test_get_profile_concurrent_lazy_create_same_new_user(pg_url):
    await apply_migrations(pg_url)
    from bot import memory
    await memory._reset_engine_for_tests()
    profiles = await asyncio.gather(
        memory.get_profile(4242),
        memory.get_profile(4242),
    )
    assert profiles[0]["telegram_user_id"] == 4242
    assert profiles[1]["telegram_user_id"] == 4242
